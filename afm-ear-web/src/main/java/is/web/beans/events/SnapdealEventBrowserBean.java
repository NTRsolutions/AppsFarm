...
@ManagedBean(name = "snapdealEventBrowserBean")
@SessionScoped
public class SnapdealEventBrowserBean implements Serializable {
	...
	public SnapdealEventBrowserBean() {
	}
		...
		try {
			...
			lazyModel = new LazyDataModel<UserEventEntity>() {
				...
				public List<UserEventEntity> load(int first, int pageSize, String sortField, SortOrder sortOrder,
					...
					filters.put("userEventCategory", UserEventCategory.SNAPDEAL.toString());
	...
	public void refresh() {
		try {
			...
			// refresh tab GUI after model update
			RequestContext.getCurrentInstance().update("tabView:idUserEventsTableSD");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void displayLogs(UserEventEntity userEvent) {
		...
		RequestContext.getCurrentInstance().update("tabView:idDialogLogsSD");
	}
	...
	public void setSelectedEventForRewardTool(UserEventEntity selectedEventForRewardTool) {
		...
		RequestContext.getCurrentInstance().update("tabView:idWidgetDialogRewardToolSD");
	}
