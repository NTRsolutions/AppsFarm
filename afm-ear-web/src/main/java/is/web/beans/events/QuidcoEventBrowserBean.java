...
@ManagedBean(name = "quidcoEventBrowserBean")
@SessionScoped
public class QuidcoEventBrowserBean implements Serializable {
		...
		try {
			...
			lazyModel = new LazyDataModel<UserEventEntity>() {
				...
				public List<UserEventEntity> load(int first, int pageSize, String sortField, SortOrder sortOrder,
					...
					filters.put("userEventCategory", UserEventCategory.QUIDCO.toString());
	...
	public void refresh() {
		try {
			...
			// refresh tab GUI after model update
			RequestContext.getCurrentInstance().update("tabView:idUserEventsTableQuidco");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void displayLogs(UserEventEntity userEvent) {
		...
		RequestContext.getCurrentInstance().update("tabView:idDialogLogsQuidco");
	}
	...
	public void setSelectedEventForRewardTool(UserEventEntity selectedEventForRewardTool) {
		...
		RequestContext.getCurrentInstance().update("tabView:idWidgetDialogRewardToolQuidco");
	}
