...
public class SentinelTabBean implements Serializable {
	...
	public void init() {
		try {
			...
			loggedUserRoleName = loggedUserRoles.get(0).getName(); // at the
																	// moment
																	// support
																	// only
																	// single
			...
			if (loggedUserRoleName.equals(UserRoles.SUPER_USER.toString())) {
				/*
				 * SingleTabBean tab = new SingleTabBean();
				 * tab.setId("SystemSettings"); tab.setTitle("SystemSettings");
				 * tab.setType(SentinelTabs.TAB_SYSTEM_SETTINGS);
				 * tab.setPage("systemSettings.xhtml"); tabList.add(tab);
				 */
			} else {
				/*
				 * SingleTabBean tab = new SingleTabBean();
				 * tab.setId("Conversions"); tab.setTitle("ConversionStats");
				 * tab.setType(SentinelTabs.TAB_CONVERSION_STATS);
				 * tab.setPage("conversionStats.xhtml"); tabList.add(tab);
				 */
			}
		} catch (Exception exc) {
	...
	public void addTab(String tabType, String tabTitle, String templateName, String tabId) {
		...
		logger.info("dynamically added tab actvice index: " + activeIndex + " tab type: " + tabBean.getType()
				+ "tab title: " + tab.getTitle());

		// triggers refresh of session beans data (e.g. list of scripts are
		// pulled to refresh data when tab is opened/switched to)
	...
	public void onTabChange(TabChangeEvent event) {
		...
		// logger.info("changed tab to: "+tabBean.getTitle()+" at index:
		// "+tabBean.getIndex());
		// triggers refresh of session beans data (e.g. list of scripts are
		// pulled to refresh data when tab is opened/switched to)
		refreshDisplayedTabDataModel(tabBean);
	...
	public void refreshDisplayedTabDataModel(SingleTabBean tabBean) {
		...
		if (tabBean.getType().startsWith(SentinelTabs.TAB_DEFAULT_PAGE)){
