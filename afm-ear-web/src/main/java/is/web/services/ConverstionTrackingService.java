...
public class ConverstionTrackingService {
	...
	public String registerEvent(@PathParam("internalTransactionId") String internalTransactionId,
		...
		try {
			...
			String dataContent = "retrieved successful conversion event: " + " internalT: " + internalTransactionId
					+ " ip: " + ipAddress + " offerProviderTransactionId: " + offerProviderTransactionId;
			...
			if (event == null) {
				Application.getElasticSearchLogger().indexLog(Application.CONVERSION_ACTIVITY, -1, LogStatus.ERROR,
						Application.CONVERSION_ACTIVITY + " status: " + RespStatusEnum.FAILED + " code: "
								+ RespCodesEnum.ERROR_UNABLE_TO_IDENTIFY_TRANSACTION);
				return "{\"response\":\" status: " + RespStatusEnum.FAILED + " code: "
						+ RespCodesEnum.ERROR_UNABLE_TO_IDENTIFY_TRANSACTION + "\"}";
			}
			boolean isServerAddressListed = externalServerManager.isServerAddressListedWithType(ipAddress,
			...
			if (!isServerAddressListed) {
				Application.getElasticSearchLogger().indexLog(Application.CONVERSION_ACTIVITY, -1, LogStatus.OK,
						Application.CONVERSION_ACTIVITY + " " + Application.EXTERNAL_SERVER_NOT_LISTED + " ipAddress :"
								+ ipAddress + " in provider table: " + event.getAdProviderCodeName()
								+ " is not listed");
				//return "{\"response\":\" status: " + RespStatusEnum.FAILED + " code: "
				//+ RespCodesEnum.ERROR_SERVER_IS_NOT_LISTED + "\"}";
			}

			// check if the same user generated conversion for the same offer
			// already - if so reject it!
				...
				if (historyConversionForSameOfferIdentified) {
					Application.getElasticSearchLogger().indexLog(Application.CONVERSION_ACTIVITY, -1, LogStatus.ERROR,
							Application.CONVERSION_ACTIVITY + " status: " + RespStatusEnum.FAILED + " code: "
									...
									+ " number of identified unique clicks: " + listFraudConversions.size() + " ip: "
									+ ipAddress + " for user with phone: " + event.getPhoneNumber() + " user id: "
									+ event.getUserId() + " offer title: " + event.getOfferTitle() + " offer provider: "
									+ event.getAdProviderCodeName() + " offer provider id: " + event.getOfferSourceId()
									+ " offer id: " + event.getOfferId());
					return "{\"response\":\" status: " + RespStatusEnum.FAILED + " code: "
							+ RespCodesEnum.ERROR_USER_FRAUD_DUPLICATE_CONVERSION_DETECTED + "\"}";
				}
			}
			...
			// check if this conversion was not already identified - if so
			// reject it
			if (event.getConversionDate() != null) {
				Application.getElasticSearchLogger().indexLog(Application.CONVERSION_ACTIVITY, realmId, LogStatus.ERROR,
						Application.CONVERSION_ACTIVITY + " status: " + RespStatusEnum.FAILED + " code: "
								+ RespCodesEnum.ERROR_DUPLICATE_CONVERSION_IDENTIFIED + " ip: " + ipAddress
								...
								+ event.getInternalTransactionId() + " phone number: " + event.getPhoneNumber()
				...
				logger.severe(
						"CONVERSION_REGISTRATION aborting conversion registration - conversion already identified for this transaction id request at: "
								...
								+ event.getInternalTransactionId() + " phone number: " + event.getPhoneNumber()
								+ " userId: " + event.getUserId() + " transaction id: "
								+ event.getInternalTransactionId());
				return "{\"response\":\" status: " + RespStatusEnum.FAILED + " code: "
						+ RespCodesEnum.ERROR_DUPLICATE_CONVERSION_IDENTIFIED + "\"}";
			} else {
				...
				Application.getElasticSearchLogger().indexLog(Application.CONVERSION_ACTIVITY, realmId, LogStatus.OK,
						Application.CONVERSION_ACTIVITY + " " + Application.CONVERSION_ACTIVITY_SUCCESS
								+ " successfully persisted conversion: internalT: " + internalTransactionId
								+ " phone number: " + event.getPhoneNumber() + " userId: " + event.getUserId());

				// create user conversion log
				Application.getElasticSearchLogger().indexUserClick(realmId, event.getPhoneNumber(), "",
						event.getDeviceType(), event.getOfferId(), event.getOfferSourceId().toLowerCase(),
						...
						UserEventCategory.INSTALL.toString(), "", "", ipAddress, event.getCountryCode(),
				...
				rewardManager.updateUserConversionHistory(event); // update
																	// conversion
																	// history
																	// (needed
																	// to filter
																	...
																	// particular
																	// user)
				return rewardManager.issueReward(realm, event, null, false); // issue
																				// reward

				/*
				 * //------------------------ send reward request to reward
				 * partner -----------------------------
				 * if(event.getRewardTypeName().equals(Application.
				 * REWARD_PROVIDER_AFA.toString())) { //------------------------
				 * send reward request to radius -----------------------------
				 * return requestRewardRadius(event);//for testing: return
				 * requestRewardLocalTesting(event); } else
				 * if(event.getRewardTypeName().equals(Application.
				 * REWARD_PROVIDER_AIR_REWARDZ_INDIA.toString())) {
				 * //------------------------ send conversion notification to AR
				 * -----------------------------
				 * //sendConversionNotificationToAB(event);
				 * //------------------------ send reward request to mode
				 * ----------------------------- return
				 * requestRewardMode(event);//for testing: return
				 * requestRewardLocalTesting(event); }
				 * if(event.getRewardTypeName().equals(Application.
				 * REWARD_PROVIDER_AIR_REWARDZ_KENYA.toString())) {
				 * //------------------------ send conversion notification to AR
				 * -----------------------------
				 * //sendConversionNotificationToAB(event);
				 * //------------------------ send reward request to mode
				 ...
				 * requestRewardLocalTesting(event); } else
				 * if(event.getRewardTypeName().equals(Application.
				 * REWARD_PROVIDER_GO_AHEAD_BRIGHTON_HOVE_UK.toString())) {
				 * //------------------------ send reward request to reward
				 * provider ----------------------------- return
				 * requestRewardGoAhead(event); } else { //if no matching reward
				 * provider identifed - trigger error
				 * Application.getElasticSearchLogger().indexLog(Application.
				 * CONVERSION_ACTIVITY, realmId, LogStatus.ERROR,
				 * Application.CONVERSION_ACTIVITY+" status: "
				 * +RespStatusEnum.FAILED+"  code: "
				 * +RespCodesEnum.ERROR_NO_REWARD_TYPE_NAME_DEFINED); return
				 * "{\"result\":\" status: "+RespStatusEnum.FAILED+"  code: "
	...
	// examle call url:
	// http://127.0.0.1:8080/ab/svc/v1/conversion?internalTransactionId=aaaab&offerProviderTransactionId=2sfsdfsafaaa
	@GET
	@Produces("application/json")
	@Path("/v1/conversion/")
	public String registerEventUsingQueryRouting(@QueryParam("internalTransactionId") String internalTransactionId,
		...
		try {
			...
			if (event == null) {
				...
				// code:
				// "+RespCodesEnum.ERROR_UNABLE_TO_IDENTIFY_TRANSACTION+"\"}";
				// for enforcing supersonic url approval
				// return "{\"response\":\" status: "+RespStatusEnum.FAILED+"
				// supersonicStatus: "+offerProviderTransactionId+":OK code:
				// "+RespCodesEnum.ERROR_UNABLE_TO_IDENTIFY_TRANSACTION+"\"}";
				...
				if (historyConversionForSameOfferIdentified) {
					Application.getElasticSearchLogger().indexLog(Application.CONVERSION_ACTIVITY, -1, LogStatus.ERROR,
							Application.CONVERSION_ACTIVITY + " status: " + RespStatusEnum.FAILED + " code: "
									+ RespCodesEnum.ERROR_USER_FRAUD_DUPLICATE_CONVERSION_DETECTED
									+ " number of identified unique clicks: " + listFraudConversions.size() + " ip: "
									+ ipAddress + " event category: " + event.getUserEventCategory()
									+ " for user with phone: " + event.getPhoneNumber() + " user id: "
									+ event.getUserId() + " offer title: " + event.getOfferTitle() + " offer provider: "
									+ event.getAdProviderCodeName() + " offer provider id: " + event.getOfferSourceId()
									+ " offer id: " + event.getOfferId());
					return "{\"response\":\" status: " + RespStatusEnum.FAILED + " code: "
			...
			if (event.getConversionDate() != null) {
				Application.getElasticSearchLogger().indexLog(Application.CONVERSION_ACTIVITY, realmId, LogStatus.ERROR,
						Application.CONVERSION_ACTIVITY + " status: " + RespStatusEnum.FAILED + " code: "
								+ RespCodesEnum.ERROR_DUPLICATE_CONVERSION_IDENTIFIED + " internalT: "
								+ internalTransactionId
								+ " aborting conversion registration - user already triggered reward request at: "
								+ event.getRewardRequestDate().toString() + " internal transaction id: "
								+ event.getInternalTransactionId() + " event category: " + event.getUserEventCategory()
								+ " phone number: " + event.getPhoneNumber() + " userId: " + event.getUserId());
				logger.severe(
						"CONVERSION_REGISTRATION aborting conversion registration - conversion already identified for this transaction id request at: "
								+ event.getConversionDate().toString() + " internal transaction id: "
	...
	private void updateUserConversionHistory(UserEventEntity event) {
		try {
			...
			conversionHistoryEntryToUpdate.setRewardStatus(event.getRewardRequestStatus());// if
																							// reward
																							// req
																							// failed
																							// -
																							...
																							// reward
																							// status
			conversionHistory.setGenerationTime(new Timestamp(System.currentTimeMillis()));// if
																							// reward
																							// req
																							// failed
																							// -
				...
				if (event.getUserEventCategory().equals(UserEventCategory.INVITE.toString())) {
					Application.getElasticSearchLogger().indexLog(Application.REWARD_ACTIVITY, event.getRealmId(),
							...
							Application.INVITATION_ACTIVITY + " " + Application.REWARD_RESPONSE_ACTIVITY
									...
									+ event.getInternalTransactionId() + " status: " + RespStatusEnum.SUCCESS
									+ " code: " + RespCodesEnum.OK_NO_CONTENT);

					InvitationEntity invitation = daoInvitation
							.findByInvitingInternalTransactionId(event.getInternalTransactionId());
					if (invitation != null) { // this transaction is associated
												// with inviting user
						invitation.setInvitingRewardResponseDate(new Timestamp(System.currentTimeMillis()));
						invitation.setInvitingRewardResponseStatus(event.getRewardResponseStatus());
						invitation.setInvitingRewardResponseStatusMessage(event.getRewardResponseStatusMessage());
						daoInvitation.createOrUpdate(invitation);
					} else {
						invitation = daoInvitation.findByInvitedInternalTransactionId(event.getInternalTransactionId());
						if (invitation != null) {
							invitation.setInvitedRewardResponseDate(new Timestamp(System.currentTimeMillis()));
							invitation.setInvitedRewardResponseStatus(event.getRewardResponseStatus());
							invitation.setInvitedRewardResponseStatusMessage(event.getRewardResponseStatusMessage());
							daoInvitation.createOrUpdate(invitation);
						}
					}
				}
	...
	private String requestRewardLocalTesting(UserEventEntity event) {
		try {
			...
			Application.getElasticSearchLogger().indexLog(Application.REWARD_ACTIVITY, event.getRealmId(), LogStatus.OK,
					Application.REWARD_ACTIVITY + " " + Application.REWARD_REQUEST_ACTIVITY + " "
							+ Application.REWARD_REQUEST_IDENTIFIED + " internalT: " + event.getInternalTransactionId()
							+ " issuing credit request for user: " + event.getUserId() + " phone: "
							+ event.getPhoneNumber());
			logger.info("credit request for user: " + event.getUserId() + " phone: " + event.getPhoneNumber()
					+ " internalTransaction: " + event.getInternalTransactionId() + " currency: "
					+ event.getOfferPayoutIsoCurrencyCode() + " payout: " + event.getOfferPayout());
			...
			event.setPhoneNumber("0786641885");
			// add reuqest header
			String urlParameters = "http://localhost:8080/ab/svc/v1/reward/" + event.getId() + "/"
					+ RespStatusEnum.SUCCESS.toString() + "/OK";
	...
	private String requestRewardTest(UserEventEntity event) {
		try {
			...
			Application.getElasticSearchLogger().indexLog(Application.REWARD_ACTIVITY, event.getRealmId(), LogStatus.OK,
					"MODE_CREDIT_ACTIVITY internalT: " + event.getInternalTransactionId() + " credit request for user: "
			...
			Application.getElasticSearchLogger().indexLog(Application.REWARD_ACTIVITY, event.getRealmId(), LogStatus.OK,
					"MODE_REQUEST_CREDIT_ACTIVITY credit successfully requested for internalT: "
							...
							+ event.getOfferPayoutIsoCurrencyCode() + " status message: " + statusMessage);
			// update event
			event.setRewardRequestDate(new Timestamp(System.currentTimeMillis()));
			event.setRewardRequestStatus(RespStatusEnum.SUCCESS.toString());
			...
			// add reuqest header
			String urlParameters = "http://localhost:8080/ab/svc/v1/reward/" + event.getInternalTransactionId() + "/"
					+ RespStatusEnum.SUCCESS.toString() + "/OK";
			...
			logger.severe(exc.toString());
			Application.getElasticSearchLogger().indexLog(Application.REWARD_ACTIVITY, -1, LogStatus.ERROR,
					"MODE_REQUEST_CREDIT_ACTIVITY internalT: " + event.getInternalTransactionId()
