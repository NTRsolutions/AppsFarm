...
@Singleton
@Startup
public class TimerSnapdealToplist {
    ...
    public void initialize(){
		try {
			...
			snapdealTriggerInterval = 35;//getPropertyInt("crfIntervals", "15");
    ...
    @Timeout
    @AccessTimeout(value = 35, unit = TimeUnit.MINUTES)
    @Lock(LockType.READ)
    public void timeout(Timer timer) {
        ...
        if(timer.getInfo().equals(TimerType.TIMER_SNAPDEAL_GET_OFFERS)) {
    		try {
    			logger.info("*** TOPLIST TIMER ****");
    			snapdealTopProductsManager.loadTopProducts();
    		} catch(Exception exc) {
    			exc.printStackTrace();
