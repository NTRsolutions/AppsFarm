...
@Singleton
@Startup
public class TimerSnapdealGetReports {
    ...
    public void initialize(){
		try {
			...
			if(isMasterServer(masterServerIp)) {
		    	...
		    	TimerConfig tc = new TimerConfig(TimerType.TIMER_SNAPDEAL_GET_REPORTS, false);
    ...
    public void timeout(Timer timer) {
        ...
        if(timer.getInfo().equals(TimerType.TIMER_SNAPDEAL_GET_REPORTS)) {
    		try {
    	        RealmEntity realmEntity = daoRealm.findByName("BPM");
    	        if(realmEntity != null && realmEntity.isSnapdealReportTimerEnabled()) {
                	Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
                			LogStatus.OK, 
                			Application.SNAPDEAL_GET_REPORTS+" retrieving reports from snapdeal api, next timer trigger: "+" next timeout: "+timer.getTimeRemaining());
                	snapdealManager.updateSnapdealReports();
    	        } else {
                	Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
                			LogStatus.OK, 
                			Application.SNAPDEAL_GET_REPORTS+" NOT PROCESSING retrieving reports from snapdeal api disabled in system, next timer trigger: "+" next timeout: "+timer.getTimeRemaining());
    	        }
    		} catch(Exception exc) {
    			exc.printStackTrace();
				...
				Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
						LogStatus.ERROR, 
						Application.SNAPDEAL_GET_REPORTS+" TIMER error: "+exc.toString());
    		}
        }
    }
