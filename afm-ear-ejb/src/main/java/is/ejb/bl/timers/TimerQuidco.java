...
public class TimerQuidco {
    ...
    public void initialize(){
		try {
			...
			if(isMasterServer(masterServerIp)) {
		    	...
		    	TimerConfig tc = new TimerConfig(TimerType.TIMER_QUIDCO_GET_DELTA, false);
		        ScheduleExpression expression = new ScheduleExpression();
				//set monitoring interval
			    expression.minute("*").minute("*").hour("*/"+ triggerInterval +"/");
    ...
    public void timeout(Timer timer) {
        ...
        if(timer.getInfo().equals(TimerType.TIMER_QUIDCO_GET_DELTA)) {
    		try {
    	        RealmEntity realmEntity = daoRealm.findByName("BPM");
    	        if(realmEntity != null && realmEntity.isQuidcoTimerEnabled()) {
    				...
    				Application.getElasticSearchLogger().indexLog(Application.QUIDCO, -1, 
    						LogStatus.OK, 
    						Application.QUIDCO_GET_DELTA+" NOT PROCESSING TIMER triggered - retrieving delta transactions from quidco disabled in system");
    	        }
