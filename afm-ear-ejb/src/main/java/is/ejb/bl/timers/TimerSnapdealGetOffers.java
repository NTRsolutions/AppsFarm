...
@Singleton
@Startup
public class TimerSnapdealGetOffers {
    ...
    private int snapdealTriggerInterval = 60; //in h
    ...
    public void initialize(){
		try {
			...
			snapdealTriggerInterval = 60;//getPropertyInt("crfIntervals", "15");
			masterServerIp = getProperty("masterServerIp", "127.0.0.1");
			if(isMasterServer(masterServerIp)) {
				//init device health monitoring timer
		    	TimerConfig tc = new TimerConfig(TimerType.TIMER_SNAPDEAL_GET_OFFERS, false);
		        ScheduleExpression expression = new ScheduleExpression();
				//set monitoring interval
			    expression.minute("*").minute("*/"+ snapdealTriggerInterval +"/").hour("*");
    ...
    public void timeout(Timer timer) {
        ...
        if(timer.getInfo().equals(TimerType.TIMER_SNAPDEAL_GET_OFFERS)) {
    		try {
    			AdProviderEntity adProvider = daoAdProvider.findByName("Snapdeal");
    			SnapdealProviderConfig adProviderConfig = serDeSnapdeal.deserialize(adProvider.getConfiguration());

    			//if(adProvider.isActive() && !TimerSnapdealGetOffers.busy) {
            	if(adProvider.isActive()) {
                    logger.info("!!!!----TIMER INVOKED timer type: "+timer.getInfo()+" next timeout: "+timer.getTimeRemaining());

                	Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
                			LogStatus.OK, 
                			Application.SNAPDEAL_GET_OFFERS_FEED+" timer invoked next timeout: "+timer.getTimeRemaining());
            			RealmEntity realm = daoRealm.findByName("BPM");
            			OfferWallEntity offerWall = new OfferWallEntity();
            			offerWall.setRealm(realm);
            			
            			snapdealAPIManager.startProcessing();
            			ArrayList<String> categoryUrlList = snapdealAPIManager.getFeedCategories(offerWall);
            			snapdealAPIManager.deleteOldOffersFeed(offerWall);
            			
      
            			
                		//for each category feed url retrieve offers and store them in db
            			
                		for(int x=0;x<categoryUrlList.size();x++) {
                		//for(int x=0;x<20;x++) {
                			snapdealAPIManager.getOffers(
                					categoryUrlList.get(x),
                					offerWall, 
                					null, 
                					null, 
                					null, 
                					adProviderConfig);
                			System.out.println("Getting: " + categoryUrlList.get(x));
                		}
                		snapdealAPIManager.getDealsOfDaysOffers(offerWall, adProviderConfig);
                		
            			
            			snapdealAPIManager.endProcessing();
                } else {
                	Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
                			LogStatus.OK, 
                			Application.SNAPDEAL_GET_OFFERS_FEED+" provider inactive or busy, busy state: "+TimerSnapdealGetOffers.busy);
                			TimerSnapdealGetOffers.busy = false;
                			Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
                        			LogStatus.OK, 
                        			Application.SNAPDEAL_GET_OFFERS_FEED+" unlocked provider, busy state: "+TimerSnapdealGetOffers.busy);
				...
				Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
						LogStatus.ERROR, 
						Application.SNAPDEAL_GET_OFFERS_FEED+" TIMER error: "+exc.toString());
    		}
        }
    }
