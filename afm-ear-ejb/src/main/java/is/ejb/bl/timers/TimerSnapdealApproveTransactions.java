...
@Singleton
@Startup
public class TimerSnapdealApproveTransactions {
    ...
    private int snapdealTriggerInterval = 24; //in h
    ...
    public void initialize(){
		try {
			...
			snapdealTriggerInterval = 24;//getPropertyInt("crfIntervals", "15");
			masterServerIp = getProperty("masterServerIp", "127.0.0.1");
			if(isMasterServer(masterServerIp)) {
				//init device health monitoring timer
		    	TimerConfig tc = new TimerConfig(TimerType.TIMER_SNAPDEAL_APPROVE_REWARDS, false);
		        ScheduleExpression expression = new ScheduleExpression();
				//set monitoring interval
			    expression.minute("*").minute("*").hour("*/"+ snapdealTriggerInterval +"/");
    ...
    public void timeout(Timer timer) {
        ...
        if(timer.getInfo().equals(TimerType.TIMER_SNAPDEAL_APPROVE_REWARDS)) {
    		try {
    	        RealmEntity realmEntity = daoRealm.findByName("BPM");
    	        if(realmEntity != null && realmEntity.isSnapdealApprovedOffersTimerEnabled()) {
                	Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
                			LogStatus.OK, 
                			Application.TIMER_SNAPDEAL_APPROVE_REWARDS+" retrieving reports from snapdeal api for reward approval, next timer trigger: "+" next timeout: "+timer.getTimeRemaining());
                	snapdealManager.updateSnapdealApprovedOffers();
    	        } else {
                	Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
                			LogStatus.OK, 
                			Application.TIMER_SNAPDEAL_APPROVE_REWARDS+" NOT PROCESSING retrieving reports from snapdeal api for reward approval disabled in system, next timer trigger: "+" next timeout: "+timer.getTimeRemaining());
    	        }
    		} catch(Exception exc) {
    			exc.printStackTrace();
				...
				Application.getElasticSearchLogger().indexLog(Application.SNAPDEAL, -1, 
						LogStatus.ERROR, 
						Application.TIMER_SNAPDEAL_APPROVE_REWARDS+" TIMER error: "+exc.toString());
    		}
        }
    }
