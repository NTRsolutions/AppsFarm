...
public class HasoffersNativexAPIManager {
	...
	public ArrayList<Offer> findAllOffers(OfferWallEntity offerWall, IndividualOfferWall individualOfferWall,
			OfferFilterManager offerFilterManager, OfferRewardCalculationManager offerRewardCalculationManager,
			HasoffersNativexProviderConfig adProviderConfig, SingleOfferWallConfiguration singleOfferWallConfig)
			throws Exception {
		...
		// String requestUrl =
		// "https://api.hasoffers.com/Apiv3/json?NetworkId="+adProviderConfig.getNetworkId()+""
		...
		// String requestUrl =
		// "https://api.hasoffers.com/Apiv3/json?NetworkId=nativex&Target=Affiliate_Offer&Method=findAll&api_key=8202723cf6cb67bb77efe3d7047e1ccdd449838540ec42671e44ead9f1ef91be&fields[]=conversion_cap&fields[]=payout_cap&fields[]=name&fields[]=description&fields[]=use_target_rules&fields[]=id&contain[1]=OfferCategory";
		// String requestUrl =
		// "https://api.hasoffers.com/Apiv3/json?NetworkId=nativex&Target=Affiliate_Offer&Method=findAll&api_key=8202723cf6cb67bb77efe3d7047e1ccdd449838540ec42671e44ead9f1ef91be&fields[]=conversion_cap&fields[]=payout_cap&fields[]=name&fields[]=description&fields[]=use_target_rules&fields[]=id&contain[0]=OfferCategory&contain[1]=Thumbnail&contain[2]=Country";
		String requestUrl = "https://api.hasoffers.com/Apiv3/json?NetworkId=" + adProviderConfig.getNetworkId()
				+ "&Target=Affiliate_Offer&Method=findAll&api_key=" + adProviderConfig.getNetworkToken()
				+ "&fields[]=conversion_cap&fields[]=payout_cap&fields[]=name&fields[]=description&fields[]=use_target_rules&fields[]=id&fields[]=currency&fields[]=preview_url&fields[]=default_payout&contain[0]=OfferCategory&contain[1]=Thumbnail&contain[2]=Country";
		...
		try {
				...
				while (it1.hasNext()) {
					...
					while (it2.hasNext()) {
						...
						if (strOfferContent.startsWith("{\"Offer\"")) {
							...
							String strOfferConuntryContent = strOfferContent.substring(startCountryIndex,
									endCountryIndex);
							...
							// logger.info("identified target device:
							// "+identifiedTargetDevice);
							...
							if (strOfferCategoryContent.contains(adProviderConfig.getCategoryName())
									&& strOfferConuntryContent
											.contains("\"" + offerWall.getTargetCountriesFilter() + "\"")) {
							...
							// index:" +endIndex);
							// strOfferContent = strOfferContent.substring(0,
							// startCatIndex-1)+"}";
							...
							if (matchesTargetDevice && matchesTargetGeoLocation) {
								...
								// logger.info("----> "+strOfferContent);
								// logger.info("C---->
								// "+strOfferCategoryContent);
								// logger.info("T---->
								// "+strOfferThumbnailContent);
								// logger.info("C---->
								// "+strOfferConuntryContent);
			...
			Application.getElasticSearchLogger().indexLog(Application.OFFER_WALL_GENERATION_ACTIVITY,
					...
					Application.OFFER_POOL_SIZE + " " + OfferProviderCodeNames.HASOFFERS_NATIVEX + " offer pool size: "
							+ listPulledOffers.size());// +" rest method called,
														// content:
														// "+requestUrl);
			// update offer stats
			offerWall.setNumberOfOffersInSelectionPool(
					offerWall.getNumberOfOffersInSelectionPool() + listPulledOffers.size());
			individualOfferWall.getListOfferStats()
					.add(OfferWallStats.number_of_offers_in_selection_pool + ":" + listPulledOffers.size());
			...
			if (listPulledOffers.size() == 0) {
				Application.getElasticSearchLogger().indexLog(Application.OFFER_WALL_GENERATION_ACTIVITY,
						offerWall.getRealm().getId(), LogStatus.WARNING,
				...
				for (int i=0;i<listPulledOffers.size();i++){
					...
					// generate unique offer id used by our system to track
					// offer conversion
					String sha1Id = DigestUtils.sha1Hex(offerWall.getRealm().getId() + offerWall.getNumberOfOffers()
							+ Math.random() * 100000 + System.currentTimeMillis() + i
							+ selectedOffer.getName() + selectedOffer.getDescription() + numberOfOffersToSelect + "");
					...
					String trackingLink = getTrackingLink(offerWall, adProviderConfig, selectedOffer.getId());
					if (trackingLink == null) {
						logger.info("Tracking link null. Skipping...");
						continue;
					}
					offerToAdd.setUrl(trackingLink);
					...
					offerToAdd.setCallToAction(""); // TODO check if call to
													// action is available?
					...
					if (selectedOffer.getThumbnail() != null) {
						...
						// imagesMap.put(ThumbnailQuality.hires.toString(),
						// thumbnail.getUrl());
					...
					isOfferAcceptedByGlobalFilter = offerFilterManager.isOfferAcceptedBasedOnGlobalFilters(offerToAdd,
							offerWall, OfferProviderCodeNames.HASOFFERS_NATIVEX);
					if (isOfferAcceptedByGlobalFilter) {
						...
						if (isOfferAcceptedByGeoFilter) {
							...
							isOfferAcceptedByDeviceFilter = offerFilterManager
									.isOfferAcceptedBasedOnTargetPlatformFilter(offerToAdd, offerWall,
											OfferProviderCodeNames.HASOFFERS_NATIVEX, supportedDevices);
						}
					}

					// ------------------------- check for duplicates starts
					// ----------------
					// check for duplicate offers and select one with higher
					// payout
					...
					if (offerFilterManager.getOfferFilterConfiguration().isRejectDuplicateOffers()) { // if
																										// filter
																										// for
																										// rejecting
																										// duplicate
																										...
																										// trigger
																										// it
						if (isOfferAcceptedByGlobalFilter && isOfferAcceptedByDeviceFilter
								&& isOfferAcceptedByGeoFilter) { // process only
																	// if offer
																	// is
																	// accepted
																	// by
																	// previous
																	// filters!
							// calculate reward value/profit/split and add to
							// offer data
							offerToAdd = offerRewardCalculationManager.calculateOfferReward(offerToAdd, offerWall,
									OfferProviderCodeNames.HASOFFERS_NATIVEX);
							isOfferDuplicate = offerFilterManager.getOfferDuplicatesDetector()
									.isOfferDuplicate(offerToAdd, offerWall);
							if (isOfferDuplicate) {
								isOfferDuplicateWithHigherPayout = offerFilterManager.getOfferDuplicatesDetector()
										.isOfferDuplicateAndWithHigherPayout(offerToAdd, offerWall);
								if (isOfferDuplicateWithHigherPayout) { // we
																		// add
																		// this
																		// offer
																		// and
																		...
																		// higher
																		// payout
									offerFilterManager.getOfferDuplicatesDetector()
											.markExistingOfferForRejection(offerToAdd, offerWall);
									offerFilterManager.getOfferDuplicatesDetector().rememberAddedOffer(offerToAdd,
											offerWall);
								} else { // offer we want to add is duplicate
											// and has lower payout than already
											// added offer - reject the current
											// offer
									String message = Application.SINGLE_OFFER_DUPLICATES_FILTERING + " "
											...
											+ " rejecting following offer as it is a duplicate: " + " offer title: "
											+ offerToAdd.getTitle() + " payout in target currency: "
											+ offerToAdd.getPayoutInTargetCurrency() + " "
											+ offerToAdd.getAdProviderCodeName();
									Application.getElasticSearchLogger().indexLog(
											Application.OFFER_WALL_GENERATION_ACTIVITY, offerWall.getRealm().getId(),
					...
					} else if (isOfferAcceptedByGlobalFilter && isOfferAcceptedByDeviceFilter
							&& isOfferAcceptedByGeoFilter) { // if not checking
																// duplicates
						// calculate reward value/profit/split and add to offer
						// data
						offerToAdd = offerRewardCalculationManager.calculateOfferReward(offerToAdd, offerWall,
								OfferProviderCodeNames.HASOFFERS_NATIVEX);
					}
					...
					// "+selectedOffer.getName()+" global filter:
					// "+isOfferAcceptedByGlobalFilter+" geo filter:
					// "+isOfferAcceptedByGeoFilter+" device filter:
					// "+isOfferAcceptedByDeviceFilter);
					if (isOfferAcceptedByGlobalFilter && isOfferAcceptedByDeviceFilter && isOfferAcceptedByGeoFilter
							&& isOfferAcceptedByDuplicatesFilter) {// add offer
																	// to the
																	// wall
						...
						Application.getElasticSearchLogger().indexLog(Application.OFFER_WALL_GENERATION_ACTIVITY,
								...
								Application.SINGLE_OFFER_CREATED + " " + OfferProviderCodeNames.HASOFFERS_NATIVEX
										+ " successfully created offer " + offerToAdd.getTitle() + " offerId: "
										+ offerToAdd.getId());
			...
			// Error: "+exc.toString()+" when retrieving/processing data from
			// offer provider");
			// throw new Exception(OfferProviderCodeNames.HASOFFERS+" Error:
			// "+exc.toString()+" when retrieving/processing data from offer
			// provider"+reqResponse);
		}

		// update offer stats
		offerWall.setNumberOfGeneratedOffers(
				offerWall.getNumberOfGeneratedOffers() + listSelectedIndividualOffers.size());
		individualOfferWall.getListOfferStats()
				.add(OfferWallStats.number_of_generated_offers + ":" + listSelectedIndividualOffers.size());
		...
		getAllOffersFileInfo(offerWall, adProviderConfig.getNetworkId(), adProviderConfig.getNetworkToken(),
				listSelectedIndividualOffers);

		return listSelectedIndividualOffers;
	}

	public void getAllOffersFileInfo(OfferWallEntity offerWall, String networkId, String networkToken,
			ArrayList<is.ejb.bl.offerWall.content.Offer> listSelectedIndividualOffers) throws Exception { // if
																											// fails
																											// then
																											// particular
																											// offer
		...
		String requestUrl = "https://api.hasoffers.com/Apiv3/json?NetworkId=" + networkId
				+ "&Target=Affiliate_OfferFile&Method=findAll&api_key=" + networkToken;
		// String requestUrl =
		// "https://api.hasoffers.com/Apiv3/json?NetworkId="+networkId+"&Target=Affiliate_Offer&Method=getThumbnail&api_key="+networkToken+"&ids[]="+selectedOfferId;
		...
		try {
			...
			// serialise response into object
			// ObjectMapper mapper = new ObjectMapper();
			// GetOfferFileInfo obj = mapper.readValue(reqResponse,
			// GetOfferFileInfo.class);
			// List<DataEntry> listDataEntry =
			// obj.getResponse().getData().getData();
			...
			while (it.hasNext()) {
				...
				while (it1.hasNext()) {
					...
					while (it2.hasNext()) {
						...
						if (nodeStr.length() > 100) {
							// logger.info("-> node c: "+nodeStr.substring(0,
							// 100));
						} else {
							...
							while (it3.hasNext()) {
								...
								try {
									if (offerStr != null && offerStr.length() > 0) {
										...
										// logger.info(offerId+ " th:
										// "+thumbnailUrl);

										// create OfferFile and fill with
										// offerId and thumbnail data
			...
			// set thumbnail urls for each offer that we selected to add to the
			// wall
			for (int i = 0; i < listSelectedIndividualOffers.size(); i++) {
				...
				// logger.info("***** checking offer:
				// "+offerToAdd.getSourceId());
				for (int j = 0; j < listOfferFileData.size(); j++) {
					...
					if (offerToAdd.getSourceId().equals(String.valueOf(offerFileData.getOffer_id()))) {
						...
						if (offerToAdd.getImage() == null) {
							// logger.info("*** identified matching id:
							// "+offerFileData.getOffer_id());
							if (offerFileData.getThumbnail() != null) {
								HashMap<String, String> imagesMap = new HashMap<String, String>();
								imagesMap.put(ThumbnailQuality.Image + "-" + 1, offerFileData.getThumbnail());
								// imagesMap.put(ThumbnailQuality.Image+"-"+2,
								// selectedOffer.getThumbnail().getThumbnail());
								offerToAdd.setImage(imagesMap);
							} else { // retrieve images via api call
								offerToAdd.setImage(null);
							}
			...
			logger.severe("Unable to retrieve all offers file info, error code: " + exc.toString()
					+ " for offer provider: " + OfferProviderCodeNames.HASOFFERS_NATIVEX);
		}
	}

	public String getTrackingLink(OfferWallEntity offerWall, HasoffersNativexProviderConfig adProviderConfig,
		...
		Application.getElasticSearchLogger().indexLog(Application.OFFER_WALL_GENERATION_ACTIVITY,
				offerWall.getRealm().getId(), LogStatus.OK, OfferProviderCodeNames.HASOFFERS_NATIVEX.toString());// +"
																													// rest
																													// method
																													// called,
																													// content:
		...
		try {
			...
			// throw new Exception(OfferProviderCodeNames.HASOFFERS_NATIVEX+
			// " Error: "+exc.toString()+" when retrieving/processing data from
			// offer provider: "+reqResponse);
		}
	}

	/*
	 * public Thumbnail requestImages(OfferWallEntity offerWall, int
	 * selectedOfferId, String offerName, String networkId, String networkToken,
	 * String methodCalled) throws Exception { //if fails then particular offer
	 * generation fails as well
	 * 
	 * Thumbnail thumbnail = null; //request images //String requestUrl =
	 * "https://api.hasoffers.com/Apiv3/json?NetworkId="+networkId+
	 * "&Target=Offer&Method=getThumbnail&NetworkToken="+networkToken+"&id="+
	 * selectedOfferId; String requestUrl =
	 * "https://api.hasoffers.com/Apiv3/json?NetworkId="+networkId+
	 * "&Target=Affiliate_Offer&Method=getThumbnail&api_key="+networkToken+
	 * "&ids[]="+selectedOfferId; logger.info("retrieving thumbnail: "
	 * +requestUrl); HttpURLConnection urlConnection = null; BufferedReader in =
	 * null; String reqResponse = "";
	 * 
	 * try { URL url = new URL(requestUrl); urlConnection =
	 * (HttpURLConnection)url.openConnection();
	 * urlConnection.setConnectTimeout(offerWall.getRealm().getConnectionTimeout
	 * () * 1000);
	 * urlConnection.setReadTimeout(offerWall.getRealm().getReadTimeout() *
	 * 1000);
	 * 
	 ...
	 * null) { in.close(); } if(urlConnection != null) {
	 * urlConnection.disconnect(); } }
	 * 
	 * //if thumbnail is not properly configured on HO or it doesn't exist -
	 * return null (offer will get rejected because it has no thumbnil) try {
	 * logger.info("image request: "+requestUrl); logger.info("image response: "
	 * +reqResponse); //serialise response into object ObjectMapper mapper = new
	 * ObjectMapper(); GetOfferThumbnail obj = mapper.readValue(reqResponse,
	 * GetOfferThumbnail.class); thumbnail =
	 * obj.getResponse().getData().getThumbnail(); } catch(Exception exc) {
	 * logger.severe("Unable to retrieve thumbnail, error code: "
	 * +exc.toString()+" for offer provider: "+OfferProviderCodeNames.HASOFFERS+
	 * " offer name: "+offerName+" with source id: "+selectedOfferId); thumbnail
	 * = null; }
	 * 
	 * return thumbnail; }
	 */
