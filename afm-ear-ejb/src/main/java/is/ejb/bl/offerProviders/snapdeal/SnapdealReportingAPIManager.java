...
@Stateless
public class SnapdealReportingAPIManager {
    	...
    	logger.info(reqResponse);
     	...
     	try {
    		...
    		Application.getElasticSearchLogger().indexLog(Application.OFFER_WALL_GENERATION_ACTIVITY, 
    				...
    				" Error: "+exc.toString()+" when retrieving/processing data from offer provider");
    	}
	...
	public void getOffers(
				String offerCategoryUrl,
				OfferWallEntity offerWall,
     	...
     	try {
    		...
    		Application.getElasticSearchLogger().indexLog(Application.OFFER_WALL_GENERATION_ACTIVITY, 
    				...
    				LogStatus.OK, 
    				Application.OFFER_WALL_GENERATION_ACTIVITY+" "+
    				OfferProviderCodeNames.SNAPDEAL+" persisting offers for category: "+categoryName+" number of offers: "+listOffers.size());
...
/*
	@Lock(LockType.READ)
	public void getOffers(OfferWallEntity offerWall,
										IndividualOfferWall individualOfferWall,
										OfferFilterManager offerFilterManager,
										OfferRewardCalculationManager offerRewardCalculationManager,
										SnapdealProviderConfig adProviderConfig) throws Exception {
		...
		TimerSnapdeal.busy = true;
		logger.info(OfferProviderCodeNames.SNAPDEAL+" setting timer busy to false: current timer status after set: "+TimerSnapdeal.busy);
    	...
    	logger.info(reqResponse);
     	...
     	try {
    		...
    		try {
        		...
        		for(int i=0;i<listPersistedOffers.size();i++) {
        			SnapdealOffersEntity offerEntity = listPersistedOffers.get(i);
        			logger.info("deleting all snapdeal old data for: "+offerEntity.getCategoryName());
        			daoSnapdealOffers.delete(offerEntity);
        		}
        		logger.info("successfully deleted all snapdeal old offers data...");
    		} catch(Exception exc) {
    			exc.printStackTrace();
        		Application.getElasticSearchLogger().indexLog(Application.OFFER_WALL_GENERATION_ACTIVITY, 
        				offerWall.getRealm().getId(), 
        				LogStatus.OK, 
        				Application.OFFER_WALL_GENERATION_ACTIVITY+" "+
        				OfferProviderCodeNames.SNAPDEAL+" error when removing previously persisted offers: "+exc.toString());
				logger.severe("error when removing previously persisted offers: "+exc.toString());
    		}
    		
    		//for each category feed url retrieve offers and store them in db 
    		for(int x=0;x<categoryUrlList.size();x++) {
        		String categoryOffersOutputInJson = getCategoryOffersUsingPost(categoryUrlList.get(x), 
        				adProviderConfig.getToken(), adProviderConfig.getId());
        		ObjectMapper mapperForOffers = new ObjectMapper();
        		CategoryOffers categoryOffersHolder = mapperForOffers.readValue(categoryOffersOutputInJson, CategoryOffers.class);
        		System.out.println("out: "+categoryOffersHolder.getProducts().size());
