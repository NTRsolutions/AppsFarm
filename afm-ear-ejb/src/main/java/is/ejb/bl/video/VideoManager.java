...
public class VideoManager {
	...
	public void processVideoRewardRequest(RealmEntity realm, String email,
			...
			String offerTitle, String applicationId, String adProvider,
			String rewardTypeName, String ipAddress, AppUserEntity user) throws Exception {

		//determine payout and currency code via rewardType and denomination model settings
		double originalCurrencyOfferPayout = 0.01; //in USD - we assume all video rewards have the same payout value in USD
		...
		//NOT USED AS IT WOULD SLOW DOWN THE SYSTEM calculate video offer reward based on denomination model
		...
		// add conversion event to conversion index in es (imitate conversion so that conversion rate is in balance)
		Application.getElasticSearchLogger().indexUserClick(realm.getId(),
				event.getPhoneNumber(), "", event.getDeviceType(),
				event.getOfferId(), event.getOfferSourceId(),
				event.getOfferTitle(), event.getAdProviderCodeName(),
	...
	public double[] getVideoPayoutValueByGeo(String countryCode) {
		if(countryCode.equals(CountryCode.KE.toString())) {
			return new double[]{0.92, 0.5, 0.42}; //payout|reward|revenue (in target currency)
		} else if(countryCode.equals(CountryCode.IN.toString())) {
		...
		} else return new double[]{0.01, 0.01, 0.01}; //payout|reward|revenue
	}
